...
test_helpers_1.testCorrect(correctCases, (x) => test_helpers_1.run(settings_1.settings, x));
test('extract_mpts_discover_cache', () => {
    expect(settings_2.extract_mpts_discover_cache('')).toStrictEqual([undefined, '']);
    expect(settings_2.extract_mpts_discover_cache('{a=1\n\n}')).toStrictEqual([undefined, '{a=1\n\n}']);
    expect(settings_2.extract_mpts_discover_cache('{a=1\n\nmpts_discover_cache="!!"\n}')).toStrictEqual(['!!', '{a=1\n\n}']);
    expect(settings_2.extract_mpts_discover_cache('{a=1\n\n  mpts_discover_cache="abc" ; \n}')).toStrictEqual(['abc', '{a=1\n\n}']);
});
test('', (done) => {
    fs_1.readFile('/Users/andrei/Code/igolgi/config-manager/packages/model/src/parsers/cfg/mv_solar_flare copy.cfg', 'utf8', (err, data) => {
        const exp = {
            'global_config': {
        ...
        const out = test_helpers_1.run(settings_1.settings, data);
